...
void init_at_startup(void)
{
  ...
  else if (settings.option[OPTION_FULLSCREEN] == 1)
  {
...
#define INITFILE_WORD_LENGTH FILE_PATH_LENGTH
  ...
  else
  {
	...
	char ch;
	while ((ch = fgetc(initfile)) != EOF)
	  fputc(ch, user_initfile);
	fclose(user_initfile);
  }

  int read_in = fread(buffer, 1, INITFILE_SIZE, initfile);

  if (ferror(initfile) || read_in == 0)
  {
  ...
  if (settings.option[OPTION_VOL_MUSIC] == 0 && settings.option[OPTION_VOL_EFFECT] == 0)
	settings.sound_on = 0;
...
int read_initfile_line(char *buffer, int buffer_length, int bpos)
{
  ...
  if (strcmp(initfile_word, "background_detail") == 0)
  {
	...
	{
	case 0:
	  settings.option[OPTION_NO_BACKGROUND] = 1;
	  break;
	case 1:
	  settings.option[OPTION_FAST_BACKGROUND] = 1;
	  break;
	default:
  ...
  if (strcmp(initfile_word, "keymap") == 0)
  {
	...
	if (bpos == -1)
	  return -1;
	if (read_number < 0 || read_number >= SPECIAL_KEYS)
	{
	...
	if (bpos == -1)
	  return -1;
	if (read_number < 1 || read_number >= ALLEGRO_KEY_MAX)
	{
  ...
  if (strcmp(initfile_word, "replace_col") == 0)
  {
	...
	if (bpos == -1)
	  return -1;
	if (read_number < 0 || read_number >= PLAYERS)
	{
	...
	if (bpos == -1)
	  return -1;
	if (read_number < 0 || read_number >= TEAM_COLS)
	{
...
int read_initfile_word(char *ifword, char *buffer, int buffer_length, int bpos)
{
  ...
  while (TRUE)
  {
	if (bpos >= buffer_length)
	  return -1;
	if (buffer[bpos] == '\0' || buffer[bpos] == '\n' || buffer[bpos] == '\r' || buffer[bpos] == ' ' || word_pos >= INITFILE_WORD_LENGTH - 1)
	{
	  ifword[word_pos] = '\0';
	  bpos++;
	  return bpos;
...
#define INITFILE_NUMBER_LENGTH 6
  ...
  *read_number = 0;
  ...
  while (TRUE)
  {
	if (bpos >= buffer_length)
	  return -1;
	if (buffer[bpos] < '0' || buffer[bpos] > '9' || number_pos >= INITFILE_NUMBER_LENGTH - 1)
	{
	  bpos++;
	  break;
	}
